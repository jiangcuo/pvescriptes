...
grub_install(){
        ...
        then
                chroot $pve_target grub-install --target loongarch64-efi --no-floppy --bootloader-id='proxmox' $newdisk  || errlog "grub install to $newdisk error !"
                cp $pve_target/boot/efi/EFI/proxmox/* $pve_target/boot/efi/EFI/BOOT/  || errlog "copy grub boot dir  error !"
                cp $pve_target/boot/efi/EFI/proxmox/grubloongarch64.efi $pve_target/boot/efi/EFI/BOOT/bootloongarch64.efi || errlog "copy grub boot file  error !"
        elif [ `arch` = "riscv64" ];
        then
                chroot $pve_target grub-install --target riscv64-efi --no-floppy --bootloader-id='proxmox' $newdisk  || errlog "grub install to $newdisk error !"
                cp $pve_target/boot/efi/EFI/proxmox/* $pve_target/boot/efi/EFI/BOOT/  || errlog "copy grub boot dir  error !"
                cp $pve_target/boot/efi/EFI/proxmox/grubriscv64.efi $pve_target/boot/efi/EFI/BOOT/bootriscv64.efi || errlog "copy grub boot file  error !"
        fi
}
...
newdisk_mount(){
        ...
        diskcheck=`echo  $newdisk |grep  -E "nvme|nbd|pmem0"`
        if [ -n "$diskcheck" ];then
            mount "$newdisk"p3 $pve_target || errlog  "mount new root error"
